...
AIF replaces unstructured natural-language chatter between agents with a compact,
...
Non-goal: AIF does not bypass semantic understanding and is not a
transport for model-internal state (for example KV cache). It is a typed
protocol for exchanging decision-relevant context.
...
AIF 用結構化封包取代 agent 之間的非結構化自然語言閒聊：一段 markdown
header（負責 routing 與身分識別）、一段 fenced JSON body（負責內容）、
以及明確的終止符。

非目標：AIF 不是用來繞過語意理解，也不是模型內部狀態（例如 KV
cache）的傳輸格式。AIF 的定位是可驗證、可路由的型別化通訊協議。
